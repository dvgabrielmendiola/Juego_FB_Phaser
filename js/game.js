...
function create() {
    ...
    backgroundDay = this.add.image(assets.scene.width, 299, assets.scene.background.day).setInteractive()
    backgroundDay.setScale(480 / backgroundDay.width);
    backgroundDay.on('pointerdown', moveBird)
    backgroundNight = this.add.image(assets.scene.width, 299, assets.scene.background.night).setInteractive()
    backgroundNight.visible = false
    backgroundNight.setScale(480 / backgroundNight.width);
    ...
    ground = this.physics.add.sprite(assets.scene.width, 500, assets.scene.ground)
...
function makePipes(scene) {
    ...
    const pipeTop = pipesGroup.create(480, pipeTopY, currentPipe.top)
    pipeTop.body.allowGravity = false

    const pipeBottom = pipesGroup.create(480, pipeTopY + 450, currentPipe.bottom)
